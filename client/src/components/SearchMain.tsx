...
function SearchMain({ type = "search" }) {
  console.log("SearchMainrendered")
  ...
  console.log(friendsData?.data.data.userFriends.friends)
          ...
          <div className="rounded-md w-3/4 h-3/4 mt-2 grid grid-cols-2 grid-flow-col gap-2 grid-rows-7 p-1">
            {
              type == "search" ?
                ...
                :
                friendsData && friendsData?.data.data.userFriends.friends.map((data: any, indx: number) => (
                  <SearchBox data={data} key={indx} count={indx + 1} search={search as string} typeOf={type} />
                ))
            }
          </div>
...
function SearchBox({ data, count, search, typeOf }: { data: ISearchbox, count: number, search: string, typeOf: string }) {
  ...
  const handleFriendButton = async (id: string) => {
    try {
  ...
  return <>
    <div className="cursor-pointer bg-primary w-full flex rounded-md items-center justify-between px-3 py-1 gap-3">
      ...
      {data.isFriend ?
        <button onClick={() => handleFriendButton(data._id)} className="bg-primary rounded-full text-md size-6 text-black hover:text-red-700 font-bold text-center">
        ...
        :
        <button onClick={() => handleFriendButton(data._id)} className="bg-primary hover:text-blue-400 rounded-full text-md size-6 text-sky-300 font-bold text-center">
