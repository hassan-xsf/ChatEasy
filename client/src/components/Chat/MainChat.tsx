...
function MainChat() {
    ...
    useEffect(() => {
        ...
        socket.on('recieveMessage', ({ msg }: { msg: string }) => {
            const data = {
                from: authData?._id,
                groupId,
                msg
    ...
    const messageGroup = async (e: FieldValues) => {
        ...
        socket.emit('sendMessage', { groupId, msg: e.message, from: authData?._id })
        reset();
    }
    ...
    return <>
        <div className="bg-white w-[80%] h-[calc(75vh-7vh)] relative">
            <MemoizedChatNavbar />
            <div className="w-full bg-white h-[calc(75vh-22vh)] overflow-y-auto relative scrollbar" ref={endOfMessagesRef}>
            ...
            <form onSubmit={handleSubmit(messageGroup)} className="ring rounded-md h-[5vh] w-[95%] mx-4 my-2 absolute bottom-0 right-0 flex items-center">
                <svg className="mx-2 size-6" viewBox="0 0 24 24" fill="#FF3951" xmlns="http://www.w3.org/2000/svg">
                    <path d="M4 12H20M12 4V20" stroke="#FF3951" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
            ...
            <div className="flex flex-col w-[95%] mx-auto justify-center">
                {
                    ...
                    <div className="flex gap-2 items-center py-1">
                        ...
                        <div className="font-sans flex flex-col tracking-tighter">
                            <span className="text-md font-bold text-black">{chatName}</span>
...
function ChatMessage({ text, align }: IChatMessage) {
    return <div className={`text-white bg-primary p-3 text-sm h-[5vh] rounded-md ${align === "end" && "self-end"}`}>{text}</div>
}
