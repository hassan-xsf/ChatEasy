...
function MainChat() {
    ...
    const messageGroup = async (e: FieldValues) => {
        ...
        reset();
        toggleEmoji(false)
        socket.emit('notifyUser', { groupId, from: authData?.username as string, toName: chatName })
    }
    const handleEmoji = (emoji: string) => {
        const value = getValues('message');
        setValue('message', value + emoji)
    }

    return <>
        <div className="bg-white w-[80%] h-[calc(98vh-8vh)] sm:h-[calc(80vh-8vh)] lg:h-[calc(75vh-7vh)] relative">
            <MemoizedChatNavbar chatName={chatName} setChatName={setChatName} />
            ...
            </div>
            {
                emoji &&
                <>
                    <div className="w-[70%] flex flex-col left-0 sm:left-6 bottom-16 sm:bottom-24 overflow-y-auto p-2 h-36 bg-white ring-1 ring-primary ring-opacity-50 absolute rounded-md">

                        <div className="grid grid-cols-6 sm:grid-cols-10 lg:grid-cols-12 2xl:grid-cols-14">
                            {
                                emojis.map((emoji, indx) => (
                                    <button onClick={() => handleEmoji(emoji)} className="hover:scale-125 hover:transition-all" key={indx}>{emoji}</button>
                                ))
                            }
                        </div>
                    </div>
            ...
            <form onSubmit={handleSubmit(messageGroup)} className="ring rounded-md h-[5vh] w-[95%] mx-4 my-2 absolute bottom-0 right-0 flex items-center">
                <div className="relative">
                    <button type="button" onClick={() => toggleEmoji(prev => !prev)}>
...
const ChatNavbar: React.FC<ChatNavbarProps> = ({ chatName, setChatName }) => {
